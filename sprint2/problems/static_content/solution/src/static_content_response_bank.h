...
namespace responseBank{
    ...
    const std::unordered_map<std::string, std::string> FILE_EXT_CONTENT_TYPE = {
            {".htm", "text/html"},
            {".html", "text/html"},
            {".css", "text/css"},
            {".txt", "text/plain"},
    ...
    bool CheckStaticContentFileNotFound(
    ...
    template <typename Body, typename Allocator>
    StringResponse MakeResponseFileNotFound(
            const http::request<Body, http::basic_fields<Allocator>>& req,
            const fs::path& static_content_root) {
    ...
    template <typename Body, typename Allocator>
    bool CheckLeaveStaticContentRootDir(
            const http::request<Body, http::basic_fields<Allocator>>& req,
            const fs::path& static_content_root) {
        fs::path static_content{static_content_root};
        std::string_view path_str = req.target().substr(1, req.target().size() - 1);
        fs::path rel_path{path_str};
        static_content = fs::weakly_canonical(static_content / rel_path);
        return !IsSubPath(static_content, static_content_root);
    };

    template <typename Body, typename Allocator>
    StringResponse MakeResponseLeaveStaticContentRootDir(
            const http::request<Body, http::basic_fields<Allocator>>& req,
            const fs::path& static_content_root) {
    ...
    http::response<http::file_body> MakeResponseGetStaticContent(
        ...
        if(FILE_EXT_CONTENT_TYPE.contains(static_content.extension())) {
            res.insert(http::field::content_type, FILE_EXT_CONTENT_TYPE.at(static_content.extension()));
        } else {
